const state = {
  ...
  selectedRole: 'staff', uploadType: 'invoice', reportType: 'daily', reportFilter: 'all', selectedReportUser: null, expandedReportDates: {}, reportCalendarMonth: '', selectedReportDate: '', squareDate: '', squarePickerOpen: false, squarePickerMonth: '', squareUserId: '', expenseFiles: [], reportFiles: [], removedReportAttachmentIds: [], editingReportId: null, reportDraft: null, pendingWeeklyReview: null, expenses: [], users: [], reports: [], squareReports: [], modal: null, busy: false
};
...
function dashboard() {
  ...
  const nav = admin ? [['overview', 'layout-dashboard', '审核概览'], ['reports', 'notebook-tabs', '日志汇报'], ['report-square', 'panels-top-left', '周日志广场'], ['people', 'users-round', '员工信息']] : [['submit', 'file-plus-2', '提交报销'], ['history', 'clock-3', '历史记录'], ['report-history', 'calendar-days', '提交记录'], ['report-square', 'panels-top-left', '周日志广场'], ['profile', 'user-cog', '我的资料']];
...
function adminPage() { if (state.view === 'people') return peoplePage(); if (state.view === 'reports') return adminReports(); if (state.view === 'report-square') return reportSquare(); return adminOverview(); }
...
function reportFileList(report) {
  ...
  return [...savedFiles, ...pendingFiles].join('') || '<div class="expense-file-empty">尚未添加附件</div>';
}
...
function setReportFiles(files) {
  ...
  const nextFiles = selected.filter(file => /\.(pdf|docx)$/i.test(file.name));
  if (selected.length !== nextFiles.length) showToast('周报审核仅支持 PDF 或 DOCX 文档。');
...
function reportSquare() {
  ...
  const users = [...new Map(state.squareReports.map(report => [report.user.id, report.user])).values()].sort((a, b) => a.name.localeCompare(b.name, 'zh-CN'));
...
function reportSquareWeeklyFiles(reports, emptyText) {
  ...
  return reports.map(report => {
    ...
    const links = attachments.length
      ? attachments.map(attachment => `<a class="report-attachment-link" href="/api/attachments/${attachment.id}" target="_blank" rel="noopener">${icon('paperclip')}<span>${escapeHtml(attachment.name)}</span></a>`).join('')
      : '<span class="muted">暂无周报文件</span>';
    return `<article class="square-weekly-file-entry"><header><div class="person"><span class="avatar tiny">${initials(report.user.name)}</span><div><strong>${escapeHtml(report.user.name)}</strong><span>${report.periodStart} 至 ${report.periodEnd}</span></div></div><time>${fmtDate(report.submittedAt)}</time></header><div class="square-file-links">${links}</div></article>`;
  }).join('');
}
...
function reportBody(report) {
  ...
  const review = report.review ? `<section class="report-review-summary"><header><span>${icon('sparkles')}智能审核</span><strong>${report.review.score} 分</strong></header><p>${escapeHtml(report.review.summary)}</p>${report.review.suggestions?.length ? `<div><span>建议</span><ul>${report.review.suggestions.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul></div>` : ''}</section>` : '';
