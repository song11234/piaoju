...
function modal() {
  ...
  if (state.modal.type === 'square-weekly-report') {
    ...
    if (!report || report.type !== 'weekly' || String(report.user.id) === String(state.user?.id)) return '';
    ...
    const docxPreviews = docxAttachments.length
      ...
      : '';
    return `<div class="modal-backdrop weekly-report-backdrop" data-backdrop><section class="modal square-weekly-report-modal document-reader-modal" role="dialog" aria-modal="true" aria-label="周志文档阅读"><div class="weekly-report-content document-reader-content">${docxPreviews || documentPreviews || '<p class="muted weekly-report-empty">该周志未附带文档。</p>'}</div></section></div>`;
  }
  if (state.modal.type === 'attachment') {
